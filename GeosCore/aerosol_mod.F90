...
!
  ...
  ! (ewl, 1/23/17)
  INTEGER :: Map_NRHAER(9)     !xxc-- change 5 to 7  to 9
...
!
    ...
    ! xxc--
    LOGICAL             :: IS_BFOCPO, IS_BFOCPI, IS_BFOCPO1, IS_BFOCPI1
    LOGICAL             :: IS_BBOCPO, IS_BBOCPI, IS_BBOCPO1, IS_BBOCPI1
    ...
    DO I = 1, State_Grid%NX
       ...
       IF ( LCARB ) THEN
          ...
          ELSE IF ( IS_OCPO ) THEN
             ...
             BFOCPO(I,J,L) = Spc(I,J,L,id_BFOCPO) * OCFOPOA(I,J) &
                             / AIRVOL(I,J,L)
             BFOCPO1(I,J,L) = Spc(I,J,L,id_BFOCPO1) * OCFOPOA(I,J) &
                             / AIRVOL(I,J,L)
             BBOCPO(I,J,L) = Spc(I,J,L,id_BBOCPO) * OCFOPOA(I,J) &
                             / AIRVOL(I,J,L)
             BBOCPO1(I,J,L) = Spc(I,J,L,id_BBOCPO1) * OCFOPOA(I,J) &
                             / AIRVOL(I,J,L)              
          ENDIF
          ...
          IF ( IS_OCPI ) THEN
             ...
             BFOCPI(I,J,L) = Spc(I,J,L,id_BFOCPI) * OCFOPOA(I,J) &
                             / AIRVOL(I,J,L)
             BFOCPI1(I,J,L) = Spc(I,J,L,id_BFOCPI1) * OCFOPOA(I,J) &
                             / AIRVOL(I,J,L)
             BBOCPI(I,J,L) = Spc(I,J,L,id_BBOCPI) * OCFOPOA(I,J) &
                             / AIRVOL(I,J,L)
             BBOCPI1(I,J,L) = Spc(I,J,L,id_BBOCPI1) * OCFOPOA(I,J) &
                             / AIRVOL(I,J,L)
          ENDIF
...
!
    ...
    !zyz24
    REAL(fp) :: TEMPOD0, TEMPSSA0, TEMPASYM0
    REAL(fp) :: TEMPOD1, TEMPSSA1, TEMPASYM1
    !zyz24
    !=================================================================
    ! RDAER begins here!
    !=================================================================
    ...
    IF ( FIRST ) THEN
       DO N = 1, NRHAER  !xc change NRHAER(7) to 9
...
110       FORMAT( '     - RDAER: Using online BCPI OCPI BCPO OCPO!' )
       ...
       DO I = 1, State_Grid%NX
          ...
          ! Brown carbon[kg/m3] !from wx  xxc--
          WAERSL(I,J,L,6) = BFOCPI(I,J,L)
          WAERSL(I,J,L,7) = BFOCPI1(I,J,L)
          WAERSL(I,J,L,8) = BBOCPI(I,J,L)
          WAERSL(I,J,L,9) = BBOCPI1(I,J,L)
          DAERSL(I,J,L,3) = BFOCPO(I,J,L)
          DAERSL(I,J,L,4) = BFOCPO1(I,J,L)
          DAERSL(I,J,L,5) = BBOCPO(I,J,L)
          DAERSL(I,J,L,6) = BBOCPO1(I,J,L)
...
120       FORMAT( '     - RDAER: Using online SALA SALC' )
    ...
    MSDENS(7) = State_Chm%SpcData(id_BFOCPI1)%Info%Density
    !xxc--
    MSDENS(8) = State_Chm%SpcData(id_BBOCPI)%Info%Density
    MSDENS(9) = State_Chm%SpcData(id_BBOCPI1)%Info%Density
    ...
    !zyz24
#ifdef RRTMG
    RTODAER(:,:,:,:,1:(NAER+2)) = 0.0d0
    RTSSAER(:,:,:,:,1:(NAER+2)) = 0.0d0
    RTASYMAER(:,:,:,:,1:(NAER+2)) = 0.0d0
#endif
    !zyz24
    ...
    DO IIWV = 1, NWVS
       ...
       DO NA = 1, NAER   
          ...
          IF ((N.GT.1) .and. (N.LE.5)) THEN
             NRT=N+2
          ELSEIF ((N.GE.6) .and. (N.LE.9)) THEN   !xxc N.LE.7 change to N.LE.9
             NRT=5                                !zyz24, corresponding to OC in RRTMG
          ELSE
             NRT=N
          ENDIF
          ...
          ! zyz24: add TEMPOD, TEMPSSA, TEMPASYM to private list if RRTMG is defined
          !$OMP PARALLEL DO                                                 &
          !xxc
...
#ifdef RRTMG
          !$OMP PRIVATE( IR, TEMPOD0, TEMPSSA0, TEMPASYM0                 ) &
          !$OMP PRIVATE(     TEMPOD1, TEMPSSA1, TEMPASYM1                 ) &
#endif
          ...
          DO I = 1, State_Grid%NX
             ...
             IF ( .not. LUCX .or. ( LUCX .and. (N.LE.NRHAER)) ) THEN
                ...
                ! Calculate BrC xxc-- 
                IF ((N.GE.6) .and. (N.LE.9)) THEN
                   ODAER(I,J,L,IWV,N) = ODAER(I,J,L,IWV,N)+ BXHEIGHT(I,J,L) *0.75d0 * &
                                    DAERSL(I,J,L,N-3) * QQAA(IWV,1,N) / &
                                    ( MSDENS(N) * REAA(1,N) * 1.0D-6 ) 
                ENDIF
             ELSE
             ...
             !
             !   RTODAER(I,J,L,IWV,NRT) = RTODAER(I,J,L,IWV,NRT) + ODAER(I,J,L,IWV,N)
             !   RTSSAER(I,J,L,IWV,NRT) = (brscat + ( RTODAER(I,J,L,IWV,NRT) &
             !                            - ODAER(I,J,L,IWV,N) )* RTSSAER(I,J,L,IWV,NRT) ) &
             !                            / RTODAER(I,J,L,IWV,NRT)
             !------------------------------------------------------------------
             ! zyz24 all these are summed as OC
             ! zyz24 summation for RTODAER; average for RTSSAER weighted by AOD
             ! zyz24 average for RTASYMAER weighted by AOD*SSA
             ELSEIF ((N .EQ. 3) .OR. (N.GE.6) .AND. (N.LE.9)) THEN
                 TEMPOD0 = RTODAER(I,J,L,IWV, NRT)
                 TEMPSSA0 = RTSSAER(I,J,L,IWV, NRT)
                 TEMPASYM0 = RTASYMAER(I,J,L,IWV,NRT)
                 
                 TEMPOD1 = ODAER(I,J,L,IWV,N)
                 TEMPSSA1 = SCALESSA*SSAA(IWV,1,N)
...
!
    ...
    !xxc++
    ALLOCATE( BFOCPI( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:BFOCPI', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
      BFOCPI = 0.0_fp

    ALLOCATE( BFOCPI1( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:BFOCPI1', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
      BFOCPI1 = 0.0_fp

    ALLOCATE( BFOCPO( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:BFOCPO', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
      BFOCPO = 0.0_fp

    ALLOCATE( BFOCPO1( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:BFOCPO1', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
      BFOCPO1 = 0.0_fp

    ALLOCATE( BBOCPI( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:BBOCPI', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
      BBOCPI = 0.0_fp

    ALLOCATE( BBOCPI1( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:BBOCPI1', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
      BBOCPI1 = 0.0_fp

    ALLOCATE( BBOCPO( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:BBOCPO', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
      BBOCPO = 0.0_fp

    ALLOCATE( BBOCPO1( State_Grid%NX, State_Grid%NY, State_Grid%NZ ), STAT=RC )
    CALL GC_CheckVar( 'aerosol_mod.F90:BBOCPO1', 0, RC )
    IF ( RC /= GC_SUCCESS ) RETURN
      BBOCPO1 = 0.0_fp
    !--------------------------------------------------------------------------
    ...
    ! Mass of hydrophobic aerosol from Mian Chin  !xxc change 2 to 6
    ALLOCATE( DAERSL(State_Grid%NX,State_Grid%NY,State_Grid%NZ,6), STAT=RC )

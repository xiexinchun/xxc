...
!
  ...
  TYPE, PUBLIC :: DgnState
     ...
     ! zyz24
     REAL(f4), POINTER :: BrCLifetime (:,:,:) ! brown carbon lifetime
     LOGICAL :: Archive_BrCLifetime
     ! zyz24
...
!
 ...
 10 FORMAT( /, 'Allocating the following fields of the State_Diag object:' )
    ...
    IF ( Input_Opt%ITS_A_FULLCHEM_SIM .or. Input_Opt%ITS_AN_AEROSOL_SIM ) THEN
       ...
       ! zyz24
       ! ------------------------------------------------------------------
       ! Brown carbon lifetime (day)
       !-------------------------------------------------------------------
       arrayID = 'State_Diag%BrCLifetime'
       diagID = 'BrCLifetime'
       CALL Check_DiagList( am_I_Root, Diag_List, diagID, Found, RC )
       IF ( Found ) THEN
          IF ( am_I_Root ) WRITE( 6, 20 ) ADJUSTL( arrayID ), TRIM( diagID )
          ALLOCATE( State_Diag%BrCLifetime( IM, JM, LM ), STAT=RC )
          CALL GC_CheckVar( arrayID, 0, RC )
          IF ( RC /= GC_SUCCESS ) RETURN
          State_Diag%BrCLifetime = 0.0_f4
          State_Diag%Archive_BrCLifetime = .TRUE.
          CALL Register_DiagField( Input_Opt, diagID,                       &
                                   State_Diag%BrCLifetime,                  &
                                   State_Chm, State_Diag, RC                )
          IF ( RC /= GC_SUCCESS ) RETURN
       ENDIF
       ! zyz24
...
!
   ...
   ENDIF
    ...
    ! zyz24
    IF ( ASSOCIATED( State_Diag%BrCLifetime ) ) THEN
       DEALLOCATE( State_Diag%BrCLifetime, STAT=RC )
       CALL GC_CheckVar( 'State_Diag%BrCLifetime', 2, RC )
       IF ( RC /= GC_SUCCESS ) RETURN
       State_Diag%BrCLifetime => NULL()
    ENDIF
    ! zyz24
...
!
    ...
    ! zyz24
    ELSE IF ( TRIM( Name_AllCaps ) == 'BRCLIFETIME' ) THEN
       IF ( isDesc    ) Desc  = 'Bleaching lifetime of brown carbon'
       IF ( isUnits   ) Units = 'days'
       IF ( isRank    ) Rank  =  3
    ! zyz24
